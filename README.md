...
사용자는 지역, 날짜, 카테고리 기준으로 팝업스토어를 탐색하고, 상세 페이지에서 위치 정보와 관련 팝업을 확인할 수 있습니다.

또한 로그인 사용자는 관심 있는 팝업스토어를 즐겨찾기에 저장하고, 서비스에 없는 팝업스토어를 직접 제보할 수 있습니다.

이 프로젝트는 단순히 팝업스토어 데이터를 화면에 보여주는 것에서 더 나아가, **네이버 지도 SDK 렌더링 안정화**, **지도 마커와 카드 리스트 동기화**, **검색 상태 URL 유지**, **페이지네이션과 스크롤 제어**, **크롤링 데이터 누락 상황에 대한 fallback UI**처럼 실제 서비스 화면에서 발생할 수 있는 프론트엔드 문제를 다루는 데 중점을 두었습니다.
...
팝업스토어 탐색 서비스에서는 위치, 이미지, 카테고리, 운영 기간 같은 정보가 함께 보여야 사용자가 쉽게 탐색할 수 있습니다.

하지만 이 프로젝트의 데이터는 크롤링 기반이기 때문에 좌표, 이미지, 설명, 카테고리 같은 정보가 일부 누락될 수 있었습니다.

또한 네이버 지도 SDK는 외부 스크립트로 로드되기 때문에 React 컴포넌트 렌더링 시점과 SDK 준비 시점이 항상 일치하지 않았습니다.

이 프로젝트에서는 다음 문제들을 중심으로 프론트엔드 흐름을 정리했습니다.
...
| Result | 새로고침이나 페이지 이동 시점에 관계없이 지도 렌더링이 안정적으로 동작하도록 개선했습니다. |
...
| Result | 지도 기반 탐색과 리스트 기반 탐색이 자연스럽게 이어지도록 개선했습니다. |
...
| Result | 일부 데이터가 불완전해도 전체 화면이 깨지지 않고 사용 가능한 상태를 유지할 수 있었습니다. |
