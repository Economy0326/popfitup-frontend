...
이 저장소는 PopFitUp 팀 프로젝트에서 제가 담당한 프론트엔드 구현을 포트폴리오용으로 분리해 정리한 저장소입니다.

원본 팀 프로젝트는 프론트엔드와 백엔드가 함께 관리된 저장소로 진행되었으며, 저는 React/Vite 기반 화면 구현, 네이버 지도 연동, 검색/필터 상태 관리, 상세/즐겨찾기/제보 화면, 예외 UI, API 연동 흐름을 담당했습니다.

백엔드 API, DB, OAuth, 세션 처리는 팀원의 구현 범위였으며, 이 저장소에서는 프론트엔드 단독 확인을 위해 mock 모드를 함께 제공합니다.

| 구분 | 설명 |
| --- | --- |
| 내 담당 | React/Vite 기반 프론트엔드, 네이버 지도 UI, 검색/필터, 상세 페이지, 즐겨찾기 UI, 제보 UI, 관리자 제보 화면, API 연동 |
| 팀 백엔드 연동 기능 | 네이버 로그인, 세션 유지, 즐겨찾기 저장, 제보 등록/조회, 관리자 답변/삭제 |
| 단독 시연 | `VITE_USE_MOCK=true` 설정으로 백엔드 없이 주요 화면 확인 가능 |
...
| Auth Flow | Session Cookie 기반 인증 상태 연동 |
...
| 실행 모드 | 설명 | 사용 설정 |
| --- | --- | --- |
| Mock 모드 | 백엔드 없이 프론트엔드 화면과 주요 UX 흐름을 확인하는 모드 | `VITE_USE_MOCK=true` |
| API 연동 모드 | 원본 팀 백엔드와 연동해 로그인, 즐겨찾기, 제보 기능까지 확인하는 모드 | `VITE_USE_MOCK=false` |

포트폴리오 검토 시에는 Mock 모드로 프론트엔드 화면 흐름을 단독 확인할 수 있습니다.  
실제 로그인, 즐겨찾기 저장, 제보 등록/조회, 관리자 답변/삭제 기능은 원본 팀 백엔드와 연동해야 동작합니다.
...
- 팝업스토어 상세 페이지
- 네이버 지도 기반 위치 표시
- 비슷한 카테고리 / 주변 지역 팝업 추천 UI
- 즐겨찾기 목록 지도 + 카드 리스트 UI
- 지도 마커 클릭 시 말풍선 표시
...
- 관리자 제보 관리 화면
- 반응형 레이아웃
- 데이터 누락 상황을 고려한 fallback UI

### 백엔드 연동 필요 기능

아래 기능은 원본 팀 프로젝트의 백엔드 API와 연동해야 실제 데이터 저장 및 인증 흐름까지 확인할 수 있습니다.

- 네이버 OAuth 로그인
- 세션 쿠키 기반 로그인 유지
- 로그인 사용자 즐겨찾기 등록/삭제/조회
- 팝업스토어 제보 등록
- 내 제보 목록 조회 및 삭제
- 관리자 제보 목록 조회
- 관리자 답변 등록
...
- 홈 화면의 최신 / 인기 / 월별 팝업 섹션
- 검색 조건 변경 및 URL query string 유지
- 검색 결과 페이지네이션
- 팝업스토어 상세 화면
- 지도와 카드 리스트 연결 흐름
- 즐겨찾기 지도 UI
- 제보 화면 UI
- 관리자 제보 관리 UI
- 이미지, 좌표, 카테고리, 설명 누락 상황의 fallback UI

> 관리자 제보 관리는 팀 프로젝트 당시 백엔드 API 스펙에 맞춰 key 입력 방식으로 구현했습니다. 이 저장소에서는 프론트엔드 화면 흐름과 관리자 제보 처리 UI를 확인하는 용도로 사용합니다.
...
제보 화면에서는 팝업스토어 이름, 위치, 설명을 입력할 수 있고, 등록 이후 내 제보 목록에서 처리 상태와 관리자 답변을 확인할 수 있도록 구성했습니다.
...
| 변수명 | 설명 |
| --- | --- |
| `VITE_USE_MOCK` | `true`이면 백엔드 없이 mock API로 화면을 확인합니다. |
| `VITE_API_URL` | `VITE_USE_MOCK=false`일 때 요청을 보낼 백엔드 API 주소입니다. |

### Mock 모드 실행

백엔드 없이 프론트엔드 화면을 확인할 때 사용합니다.
...
운영 또는 배포 환경에서는 `VITE_API_URL`을 실제 백엔드 API 주소로 변경합니다.
...
이 저장소는 PopFitUp 팀 프로젝트에서 제가 담당한 프론트엔드 작업을 포트폴리오용으로 분리한 저장소입니다.

백엔드 코드는 포함하지 않으며, `VITE_USE_MOCK=true` 설정을 통해 백엔드 없이도 주요 화면과 UX 흐름을 확인할 수 있도록 구성했습니다.
